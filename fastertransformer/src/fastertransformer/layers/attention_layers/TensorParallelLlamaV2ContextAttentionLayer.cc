...
template<typename T>
void TensorParallelLlamaV2ContextAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                                            ...
                                                            const AttentionWeight<T>* attention_weights)
{
    ...
    LlamaV2ContextAttentionLayer<T>::forward(output_tensors, input_tensors, attention_weights);
    ...
    if (do_all_reduce_ && tensor_para_.world_size_ > 1) {
        if (!use_custom_all_reduce_kernel) {
            ftNcclAllReduceSum(
                attention_out, attention_out, size, tensor_para_, LlamaV2ContextAttentionLayer<T>::stream_);
        }
        else {
            custom_all_reduce_comm_->customAllReduce(size, LlamaV2ContextAttentionLayer<T>::stream_);
        }
...
template<typename T>
TensorParallelLlamaV2ContextAttentionLayer<T>::TensorParallelLlamaV2ContextAttentionLayer(
    ...
    LlamaV2ContextAttentionLayer<T>(max_batch_size,
                                    max_seq_len,
                                    head_num,
                                    kv_head_num,
                                    size_per_head,
...
template<typename T>
TensorParallelLlamaV2ContextAttentionLayer<T>::TensorParallelLlamaV2ContextAttentionLayer(
    TensorParallelLlamaV2ContextAttentionLayer<T> const& attention_layer):
    LlamaV2ContextAttentionLayer<T>(attention_layer),
...
template class TensorParallelLlamaV2ContextAttentionLayer<float>;
template class TensorParallelLlamaV2ContextAttentionLayer<half>;
#ifdef ENABLE_BF16
template class TensorParallelLlamaV2ContextAttentionLayer<__nv_bfloat16>;
#endif
