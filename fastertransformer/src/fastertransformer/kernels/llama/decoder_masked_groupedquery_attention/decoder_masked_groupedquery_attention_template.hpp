...
template<
    ...
    int  THREADS_PER_BLOCK,
    bool HAS_BEAMS>
__global__ void groupedquery_attention_masked_kernel(GroupedQuery_attention_params<T> params)
{
    using Tk = typename kernel_type_t<T>::Type;
#ifdef ENABLE_FP8
    // FP8 MHA Scales
    ...
    if (params.rotary_embedding_dim > 0 && !params.neox_rotary_style) {
        if (handle_kv) {
            apply_rotary_embedding(q, k, tidx, params.rotary_embedding_dim, params.timestep - padd_len, params.rope_theta, params.rope_scaling);
        }
        else {
            apply_rotary_embedding(q, tidx, params.rotary_embedding_dim, params.timestep - padd_len, params.rope_theta, params.rope_scaling);
        }
    }
        ...
        if (do_rotary) {
            ...
            if (handle_kv) {
                ...
                mmha::apply_rotary_embedding(
                    q, k, transpose_idx / tidx_factor, params.rotary_embedding_dim, params.timestep - padd_len, params.rope_theta, params.rope_scaling);

                mmha::write_smem_transpose(k, k_smem, transpose_idx, smem_pitch);
            }
            else {
                mmha::apply_rotary_embedding(
                    q, transpose_idx / tidx_factor, params.rotary_embedding_dim, params.timestep, params.rope_theta, params.rope_scaling);
            }
