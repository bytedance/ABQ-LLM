...
template<typename T>
class SeedGPTContextDecoder: public BaseLayer {
private:
    // buffer handling
    ...
    T*      decoder_layer_lora_output_    = nullptr;
...
protected:
public:
    SeedGPTContextDecoder(size_t                              max_batch_size,
                          size_t                              max_seq_len,
                          size_t                              head_num,
    ...
    SeedGPTContextDecoder(SeedGPTContextDecoder<T> const& decoder);

    ~SeedGPTContextDecoder();

    void forward(TensorMap*                                            output_tensors,
                 const TensorMap*                                      input_tensors,
                 const std::vector<SeedGPTDecoderLayerWeight<T>*>* decoder_layer_weights);
};
