...
template<typename T>
struct LlamaV2Weight {

    LlamaV2Weight() = default;
    LlamaV2Weight(
        const int                                  head_num,
    ...
    ~LlamaV2Weight();
    LlamaV2Weight(const LlamaV2Weight& other);
    LlamaV2Weight& operator=(const LlamaV2Weight& other);

    void loadModel(std::string dir_path);

    void resizeLayer(const int num_layer);

    std::vector<LlamaV2DecoderLayerWeight<T>*> decoder_layer_weights;
