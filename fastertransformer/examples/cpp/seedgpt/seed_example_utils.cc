...
*/
#include "examples/cpp/seedgpt/seed_example_utils.h"
...
template<typename T>
int read_start_embedding(size_t              batch_size,
                        T*&                 input_embeddings,
                        std::vector<int>*   v_start_lengths,
                        std::vector<int>*   v_start_mask,
                        size_t&             max_input_len,
                        size_t              input_sys_len,
                        ...
                        std::string         file_name)
{
   ...
   for (const auto& entry : std::experimental::filesystem::directory_iterator(file_name)) {
       if (entry.path().extension() == ".bin") {
           embedding_files.push_back(file_name + "/" + entry.path().filename().string());
           FILE *fp = fopen(embedding_files.back().c_str(), "r+");
           if (!fp) return 0;
           fseek(fp, 0, SEEK_END);
           int cur_length = ftell(fp);
           v_start_lengths->push_back(cur_length / embed_size / sizeof(T));
           max_input_len = max_input_len > cur_length ? max_input_len : cur_length;
           fseek(fp, 0, SEEK_SET);
           fclose(fp);
       }
   }

   batch_size = batch_size == 0 ? embedding_files.size() : batch_size;
   input_embeddings = (T *)malloc(max_input_len * batch_size);
   memset(input_embeddings, 0, max_input_len * batch_size);
   max_input_len = max_input_len / embed_size / sizeof(T);

   for(int i = 0; i < batch_size; i++) {
       if (batch_size <= v_start_lengths->size()) {
           FILE *fp = fopen(embedding_files[i].c_str(), "r+");
           fread(input_embeddings + i * embed_size * max_input_len, embed_size * sizeof(T) * (*v_start_lengths)[i], 1, fp);
           fclose(fp);
       }
   }

   *v_start_mask = std::vector<int>(max_input_len * batch_size, 0);
   for (int i = 0; i < batch_size; i++) {
       for (int j = input_sys_len; j < input_sys_len + input_image_len; j++) {
           (*v_start_mask)[j] = 1;
       }
   }
...
model_config_t read_model_config(const INIReader& reader)
{
   ...
   config.hidden_units   = reader.GetInteger(config.model_name, "hidden_units");//config.head_num * config.size_per_head;
   config.inter_size     = reader.GetInteger(config.model_name, "inter_size", 4 * config.hidden_units);
   config.embed_size     = config.head_num * config.size_per_head;
...
request_config_t read_request_config(const INIReader& reader)
{
   ...
   config.request_sys_len = reader.GetInteger("request", "request_sys_len");
   config.request_image_len = reader.GetInteger("request", "request_image_len");
   ...
   config.receive_input_ids    = reader.GetBoolean("request", "receive_input_ids", false);
   ...
   config.lora_r = reader.GetInteger("request", "lora_r");
   config.lora_start_layer = reader.GetInteger("request", "lora_start_layer");
   return config;
}
...
template<typename T>
void populate_request_embedding(std::unordered_map<std::string, Tensor>& input_tensors,
                               std::unordered_map<std::string, Tensor>& output_tensors,
                               T*&                             d_input_embedding,
                               int*&                           d_input_lora_mask,
                               int*&                           d_input_lengths,
                               std::vector<int>&               v_start_lengths,
                               std::vector<int>&               output_seq_len,
                               std::vector<int>&               p_prompt_tuning_task_name_ids,
                               ...
                               const request_config_t&         request_config)
{
   ...
   const auto beam_width         = request_config.beam_width;
   const auto request_output_len = request_config.request_output_len;
   const auto embedding_size = model_config.embed_size;

   T* input_embedding = nullptr;
   std::vector<int> v_start_mask;
   request_batch_size = read_start_embedding(request_batch_size, input_embedding, &v_start_lengths, &v_start_mask,
                                             max_input_len, request_config.request_sys_len, request_config.request_image_len,embedding_size, model_config.end_id, beam_width, csv_path);

   if (max_input_len > 0) {
       // conditional case.
       deviceMalloc(&d_input_embedding, request_batch_size * max_input_len * beam_width * embedding_size, false);
       deviceMalloc(&d_input_lengths, request_batch_size, false);
       deviceMalloc(&d_input_lora_mask, request_batch_size * max_input_len, false);

       cudaH2Dcpy(d_input_embedding, input_embedding, request_batch_size * max_input_len * beam_width * embedding_size);
       cudaH2Dcpy(d_input_lengths, v_start_lengths.data(), request_batch_size);
       if(!v_start_mask.empty()) {
           cudaH2Dcpy(d_input_lora_mask, v_start_mask.data(), request_batch_size * max_input_len);
       }
   ...
   output_seq_len = std::vector<int>(request_batch_size, total_output_len);
   if (sizeof(T) == 4) {
       input_tensors.insert({"input_embedding", {MEMORY_GPU, TYPE_FP32, {request_batch_size * beam_width, max_input_len, embedding_size}, d_input_embedding}});
   } else {
       input_tensors.insert({"input_embedding", {MEMORY_GPU, TYPE_FP16, {request_batch_size * beam_width, max_input_len, embedding_size}, d_input_embedding}});
   }
   if (request_config.request_image_len) {
       input_tensors.insert({"input_lora_mask", {MEMORY_GPU, TYPE_INT32, {request_batch_size, max_input_len}, d_input_lora_mask}});
   }
   input_tensors.insert({"lora_r", {MEMORY_CPU, TYPE_INT32, {1}, &request_config.lora_r}});
   input_tensors.insert({"lora_start_layer", {MEMORY_CPU, TYPE_INT32, {1}, &request_config.lora_start_layer}});
   input_tensors.insert({"input_lengths", {MEMORY_GPU, TYPE_INT32, {request_batch_size}, d_input_lengths}});
   input_tensors.insert({"input_lengths_h", {MEMORY_CPU, TYPE_INT32, {request_batch_size}, v_start_lengths.data()}});
   input_tensors.insert({"output_seq_len", {MEMORY_CPU, TYPE_UINT32, {request_batch_size}, output_seq_len.data()}});
   input_tensors.insert({"input_sys_len", {MEMORY_CPU, TYPE_INT32, {1}, &request_config.request_sys_len}});
   input_tensors.insert({"temperature", {MEMORY_CPU, TYPE_FP32, {1}, &request_config.temperature}});
   input_tensors.insert({"len_penalty", {MEMORY_CPU, TYPE_FP32, {1}, &request_config.len_penalty}});
   if (request_config.repetition_penalty != 1.0f) {
       input_tensors.insert({"repetition_penalty", {MEMORY_CPU, TYPE_FP32, {1}, &request_config.repetition_penalty}});
...
#define INSTANTIATE__POPULATE_REQUEST_EMBED(T)                                                          \
template void populate_request_embedding(std::unordered_map<std::string,  Tensor>& input_tensors,       \
                                        std::unordered_map<std::string,  Tensor>& output_tensors,      \
                                        T*&                              d_input_embedding,            \
                                        int*&                            d_input_lora_mask,            \
                                        int*&                            d_input_lengths,              \
...
INSTANTIATE__POPULATE_REQUEST_EMBED(half);
INSTANTIATE__POPULATE_REQUEST_EMBED(float);
#ifdef ENABLE_BF16
INSTANTIATE__POPULATE_REQUEST_EMBED(__nv_bfloat16);
#endif
