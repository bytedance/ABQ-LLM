...
template<typename T>
class FTLlama2: public IFLlama2 {
public:
    FTLlama2(const size_t             head_num,
                 ...
                 th::optional<int64_t>    return_cum_log_probs_opt) override
    {
        ...
        ft::LlamaV2<T> llama = ft::LlamaV2<T>(head_num_,
                                              kv_head_num_,
                                              size_per_head_,
                                              inter_size_,
                                              layer_num_,
        ...
        if (bad_words_opt.has_value()) {
            input_tensors.insert(
                {"bad_words_list",
                 convert_tensor<int32_t>(bad_words_opt.value(), ft::MemoryType::MEMORY_GPU)});
        }
        if (stop_words_opt.has_value()) {
            input_tensors.insert(
                {"stop_words_list",
                 convert_tensor<int32_t>(stop_words_opt.value(), ft::MemoryType::MEMORY_GPU)});
        }
...
private:
    ...
    ft::LlamaV2Weight<T>    Llama_weights_;
...
class Llama2Op: public th::jit::CustomClassHolder {
public:
    Llama2Op(const int64_t            head_num,
