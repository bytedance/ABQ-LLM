...
#include "src/fastertransformer/models/llamaV2/LlamaV2Weight.h"

namespace fastertransformer {

template<typename T>
LlamaV2Weight<T>::LlamaV2Weight(const int                                  head_num,
    ...
    prompt_learning_pair_(prompt_learning_pair)
{
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(new LlamaV2DecoderLayerWeight<T>(head_num_,
                                                                             kv_head_num_,
                                                                             size_per_head_,
                                                                             inter_size_,
                                                                             tensor_para_size_,
            ...
            decoder_layer_weights.push_back(new LlamaV2DecoderLayerWeight<T>(0, 0, 0, 0));
        }
    }
...
template<typename T>
LlamaV2Weight<T>::LlamaV2Weight(const LlamaV2Weight& other):
...
template<typename T>
LlamaV2Weight<T>& LlamaV2Weight<T>::operator=(const LlamaV2Weight& other)
{
    head_num_                   = other.head_num_;
    kv_head_num_                = other.kv_head_num_;
    size_per_head_              = other.size_per_head_;
...
template<typename T>
void LlamaV2Weight<T>::setWeightPtr()
{
...
template<typename T>
void LlamaV2Weight<T>::mallocWeights()
{
...
template<typename T>
void LlamaV2Weight<T>::loadModel(std::string dir_path)
{
...
template<typename T>
void LlamaV2Weight<T>::resizeLayer(const int num_layer)
{
    num_layer_ = num_layer;
    decoder_layer_weights.reserve(num_layer_);
    for (int l = 0; l < num_layer_; l++) {
        decoder_layer_weights.push_back(new LlamaV2DecoderLayerWeight<T>());
    }
}

template<typename T>
bool LlamaV2Weight<T>::isValidLayerParallelId(int l)
{
...
template struct LlamaV2Weight<float>;
template struct LlamaV2Weight<half>;
#ifdef ENABLE_BF16
template class LlamaV2Weight<__nv_bfloat16>;
#endif
