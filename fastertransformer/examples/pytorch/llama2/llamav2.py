...
class LlamaWeights(object):
    def __init__(self,
                 head_num, kv_head_num, size_per_head, inter_size, layer_num, vocab_size,
        ...
        if self.kv_head_num is None:
            self.kv_head_num = self.head_num

        qkv_local_head_num = (head_num + 2 * kv_head_num) // tensor_para_size
        qkv_local_hidden_units = qkv_local_head_num * size_per_head
        
        global_hidden_units = head_num * size_per_head
        local_hidden_units = global_hidden_units // tensor_para_size 
        ...
        self.w.extend([torch.zeros(global_hidden_units, qkv_local_hidden_units, dtype=self.inference_data_type)] * layer_num)   # 2 self_attention_weights.query_weight.kernel
        ...
        self.w.extend([torch.zeros(global_hidden_units, dtype=self.inference_data_type)] * layer_num)                           # 11 ffn_weights.output_weight.bias
        self.w.extend([torch.zeros(global_hidden_units, dtype=self.inference_data_type)] * layer_num)                           # 12 post_attention_layernorm_weights.beta
...
class LlamaV2(nn.Module):
    def __init__(self,
                 head_num, kv_head_num, size_per_head, inter_size,
                 vocab_size, rotary_embedding_dim, rope_theta, rope_scale, 
                 layernorm_eps,
        ...
        assert tensor_para_size * pipeline_para_size == 1, "just support single gpu inference!"
        ...
        # #old bytenn ft lib
        try:
            true_lib_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "lib/libth_transformer_abi0.so")
            torch.classes.load_library(true_lib_path)
        except OSError:
            try:
                true_lib_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "lib/libth_transformer.so")
                torch.classes.load_library(true_lib_path)
            except OSError:
                raise NotImplementedError("libth_transformer is incompatible with the torch version, please contact liusongwei.zju@bytedance.com to recompile!")
        if lib_path is not None:
            print("WARNING: Library loading does not accept external incoming paths, so {lib_path} does not take effect")

        # Prepare weights
        self.weights = LlamaWeights(head_num, kv_head_num, size_per_head, inter_size, layer_num, vocab_size,
                                        max_seq_len, tensor_para_size, pipeline_para_size, use_gptj_residual,
    ...
    def cuda(self):
        ...
        self.model = torch.classes.FasterTransformer.Llama2Op(self.head_num, self.kv_head_num, self.size_per_head, self.inter_size,
                                                             self.layer_num, self.vocab_size, self.rotary_embedding_dim, 
                                                             self.rope_theta, self.rope_scale, self.layernorm_eps,
...
class ParallelLlamaV2(nn.Module):
    def __init__(self,
                 head_num, kv_head_num, size_per_head, inter_size,
                 vocab_size, rotary_embedding_dim, rope_theta, rope_scale, 
                 layernorm_eps,
        ...
        # Load the C++ model into Pytorch model.
        # #old bytenn ft lib
        # true_lib_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "libth_transformer_old.so")
        true_lib_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "lib/tp/libth_transformer.so")
        if lib_path is not None:
            print("WARNING: Library loading does not accept external incoming paths, so {lib_path} does not take effect")
        torch.classes.load_library(true_lib_path)
