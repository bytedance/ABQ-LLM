...
#include "src/fastertransformer/kernels/abqgemm/common/memory.h"
                                           ...
                                           const int *W, int *shared_mem_workspace)
{
    ...
    wmma::fragment<wmma::accumulator, MMA_M, MMA_N, MMA_K, int> cfrag[WARP_M_TILES][WARP_N_TILES];
#pragma unroll
    for (int i = 0; i < WARP_M_TILES; i++)
#pragma unroll
        for (int j = 0; j < WARP_N_TILES; j++)
            wmma::fill_fragment(cfrag[i][j], 0);
...
#pragma unroll
                for (int n = 0; n < WARP_N_TILES; n++) {
                    wmma::bmma_sync(cfrag[m][n], afrag[k % kWarpStage][m], bfrag[k % kWarpStage][n],
                                    cfrag[m][n], bmmaBitOpAND);
                }
            }
        }
        pipe.releaseReader();
...
#pragma unroll
                    for (int m = 0; m < WARP_M_TILES; m++) {
                        int offset = (x_row + m * MMA_M) * smem_ldx + k * MMA_K / 32;
                        wmma::load_matrix_sync(afrag[k % kWarpStage][m], shared_tile_x + offset,
                                               BLOCK_K);
                    }
                    // Load into bfrag
#pragma unroll
                    for (int n = 0; n < WARP_N_TILES; n++) {
                        int offset = (w_row + n * MMA_N) * smem_ldw + k * MMA_K / 32;
                        wmma::load_matrix_sync(bfrag[k % kWarpStage][n], shared_tile_w + offset,
                                               BLOCK_K);
                    }
#pragma unroll
                    for (int m = 0; m < WARP_M_TILES; m++) {
#pragma unroll
                        for (int n = 0; n < WARP_N_TILES; n++) {
                            wmma::bmma_sync(cfrag[m][n], afrag[k % kWarpStage][m],
                                            bfrag[k % kWarpStage][n], cfrag[m][n], bmmaBitOpAND);
                        }
                    }
                }
            }
...
template <
    // quant info
    typename QuantType, typename ThreadBlockShape, typename WarpShape, typename MmaShape,
    int kThreadBlockStage, typename AccumulatorType, typename ASwizzle, typename BSwizzle,
    typename CSwizzle, bool UseRegisterDoubleBuffer, bool UseMinimumSync, bool GridMappingXYToMN>
__device__ __forceinline__ void
AqBWMMAKernel<QuantType, ThreadBlockShape, WarpShape, MmaShape, kThreadBlockStage, AccumulatorType,
              ASwizzle, BSwizzle, CSwizzle, UseRegisterDoubleBuffer, UseMinimumSync,
