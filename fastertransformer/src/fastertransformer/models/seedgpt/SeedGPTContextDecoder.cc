...
#include "src/fastertransformer/models/seedgpt/SeedGPTContextDecoder.h"
...
template<typename T>
void SeedGPTContextDecoder<T>::initialize()
{
...
template<typename T>
void SeedGPTContextDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void SeedGPTContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool use_shared_contexts)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    ...
    decoder_layer_lora_output_ = reinterpret_cast<T*>(
        allocator_->reMalloc(decoder_layer_lora_output_, sizeof(T) * batch_size * seq_len * hidden_units_, false));
    if (int8_mode_ == 2) {
        // dynamic scale
...
template<typename T>
void SeedGPTContextDecoder<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        ...
        allocator_->free((void**)(&decoder_layer_lora_output_));
...
template<typename T>
bool SeedGPTContextDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool SeedGPTContextDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool SeedGPTContextDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int SeedGPTContextDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
SeedGPTContextDecoder<T>::SeedGPTContextDecoder(size_t               max_batch_size,
                                                size_t               max_seq_len,
                                                size_t               head_num,
...
template<typename T>
SeedGPTContextDecoder<T>::SeedGPTContextDecoder(SeedGPTContextDecoder<T> const& decoder):
...
template<typename T>
SeedGPTContextDecoder<T>::~SeedGPTContextDecoder()
{
...
template<typename T>
void SeedGPTContextDecoder<T>::forward(
    TensorMap*                                            output_tensors,
    const TensorMap*                                      input_tensors,
    const std::vector<SeedGPTDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    // input tensors:
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            T attn[5120];
            SeedGPTDecoderLayerWeight<T>* layer_weight = gpt_decoder_layer_weight->at(l);
            ...
            if (layernorm_type_ == LayerNormType::pre_layernorm) {
                invokeGeneralLayerNorm(decoder_normed_input_,
                                       ...
                                       h_token_num,
                                       hidden_units_,
                                       const_cast<float*>(layer_weight->self_attention_weights.query_weight.scale), //int8 mode
                                       nullptr,
...
//            cudaD2Hcpy(attn, decoder_normed_input_, hidden_units_);
//            printPerLine((half*)attn,hidden_units_,"attn (ln_1):");
//            cudaD2Hcpy(attn, decoder_normed_input_ + (seq_len-1) * hidden_units_, hidden_units_);
//            printPerLine((half*)attn,hidden_units_,"attn (ln_1)(-1):");
...
//            cudaD2Hcpy(attn, normed_self_attn_output_, hidden_units_);
//            printPerLine((half*)attn, hidden_units_, "ln2: ");
//            cudaD2Hcpy(attn, normed_self_attn_output_ + (seq_len-1)*hidden_units_, hidden_units_);
//            printPerLine((half*)attn, hidden_units_, "ln2(-1): ");
//            printf("seq_len : %d\n", seq_len);
            ...
            if (input_tensors->isExist("lora_r")) {
                if (input_tensors->at("lora_r").getVal<int32_t>() == -1) {
                    ffn_input_tensors.insert("lora_mask", Tensor{MEMORY_GPU, TYPE_INT32, {batch_size, h_token_num}, input_tensors->at("lora_mask").getPtr<int32_t>()});
                    lora_r = input_tensors->at("lora_r").getVal<int>();
                    ffn_input_tensors.insert("lora_r", Tensor{MEMORY_CPU, TYPE_INT32, {1}, &(lora_r)});
                    lora_layer = input_tensors->at("lora_start_layer").getVal<int>();
                    ffn_input_tensors.insert("lora_start_layer", Tensor{MEMORY_CPU, TYPE_INT32, {1}, &lora_layer});

                }
            }
            ffn_input_tensors.insert("cur_layer", Tensor{MEMORY_CPU, TYPE_UINT32, {1}, &l});
            ...
            if (!use_moe) {
                ...
                if (lora_r == -1) {
                    ffn_output_tensors.insert(
                        "ffn_lora_output",
                        Tensor{MEMORY_GPU, activation_out_type, {h_token_num, hidden_units_}, decoder_layer_lora_output_});
                }
            ...
            if (has_adapters_) {
                ...
                if (!use_moe) {
                    ...
                    if (lora_r == -1) {
                        ffn_output_tensors.insert(
                            "ffn_lora_output", Tensor{MEMORY_GPU, data_type, {h_token_num, hidden_units_}, decoder_output});
                    }
                }
                else {
            ...
            if (!use_moe) {
                if (layernorm_type_ == LayerNormType::pre_layernorm) {
                    if (lora_r != -1 || l < lora_layer) {
                        ...
                        invokeLoraMaskAddBiasResidual(decoder_output, decoder_output, decoder_layer_lora_output_, lora_mask,
                                                      after_adapter_attn_output_,
                                                      has_adapters_ ? ffn_output_ptr : nullptr,
                                                      has_adapters_ ? layer_weight->after_ffn_adapter_weights.output_weight.bias :
                                                                      layer_weight->ffn_weights.output_weight.bias,
                                                      layer_weight->ffn_weights.output_lora_weight.bias,
                                                      nullptr,
                                                      nullptr,
                                                      h_token_num,
                                                      hidden_units_,
...
template class SeedGPTContextDecoder<float>;
template class SeedGPTContextDecoder<half>;
#ifdef ENABLE_BF16
template class SeedGPTContextDecoder<__nv_bfloat16>;
#endif
