...
import os
import sys
sys.path.append("/opt/tiger/Video-MiniGPT-4")
from transformers import GPT2Model # transformers-4.10.0-py3
from transformers import GPT2LMHeadModel
from minigpt4.models.gpt2.gpt2_eval import GPT2LabLMHeadModel
...
def split_and_convert(args):
    ...
    model = GPT2LabLMHeadModel.from_pretrained(args.in_file, ignore_mismatched_sizes=False).to(torch_device)
    ...
    try:
        ...
        n_embd = hf_config["n_embed"]
        config["gpt"]["size_per_head"] = str(n_embd // hf_config["n_head"])
        config["gpt"]["inter_size"] = str(n_inner)
    ...
    for name, param in model.named_parameters():
        ...
        elif name == 'transformer.pre_token_proj.bias':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(saved_dir + "model.pre_token_proj.bias.bin")
        elif name == 'transformer.pre_token_proj.weight':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(saved_dir + "model.pre_token_proj.weight.bin")
        elif name == 'transformer.post_token_proj.bias':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(saved_dir + "model.pre_token_proj.bias.bin")
        elif name == 'transformer.post_token_proj.weight':
            param.detach().cpu().numpy().astype(np_weight_data_type).tofile(saved_dir + "model.pre_token_proj.weight.bin")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-processes", "-p", type=int, help="How many processes to spawn for conversion (default: 4)", default=8)
