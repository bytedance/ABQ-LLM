...
#include "wmma_base/wmma_w4a4.cuh"
...
int run(int m, int n, int k)
{
    ...
    // Launch APMM kernel
    for (int trial = 0; trial < NUM_PROFILE; trial++) {
        err = launch_pack4(d_a, d_a_pack, m, k, 4);
        if (err != cudaSuccess) {
            printf("Line %d: 'launch_pack4' failed: %s\n", __LINE__, cudaGetErrorString(err));
        }
        err = apmm_w4a4_cuda<std::is_same<ElementInputA, cutlass::int4b_t>::value>(
            d_a_pack, d_w_pack, d_out, m, n, k);
        if (err != cudaSuccess) {
            printf("Line %d: 'apmm_w4a4_cuda' failed: %s\n", __LINE__, cudaGetErrorString(err));
        }
    }
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);
    ...
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("BASE-AQGEMM (%d-bit). m: %6d, n: %6d, k: %6d,\t Time (ms): %.4f, TOPS: %4.4f\t\n", 4, m,
