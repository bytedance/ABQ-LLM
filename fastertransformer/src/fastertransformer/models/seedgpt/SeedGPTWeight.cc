...
#include "src/fastertransformer/models/seedgpt/SeedGPTWeight.h"

namespace fastertransformer {

template<typename T>
SeedGPTWeight<T>::SeedGPTWeight(const int                                  embed_size,
    ...
    gpt_variant_params_(gpt_variant_params)
{
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(new SeedGPTDecoderLayerWeight<T>(
               embed_size_, hidden_units_, inter_size_, tensor_para_size_, tensor_para_rank_, false, int8_mode_, gpt_variant_params_));
        }
        else {
...
SeedGPTWeight<T>::SeedGPTWeight(const int                                  embed_size,
                                ...
                                const int                                  layer_para_size,
                                const int                                  layer_para_rank,
                                const bool                                 moe_lora_mode,
                                const int                                  lora_start_layer,
                                const int                                  int8_mode,
                                PromptLearningType                         prompt_learning_type,
                                std::map<std::string, std::pair<int, int>> prompt_learning_pair,
                                gptVariantParams                           gpt_variant_params):
    ...
    gpt_variant_params_(gpt_variant_params)
{
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            bool flag = moe_lora_mode && lora_start_layer >= l;
            decoder_layer_weights.push_back(new SeedGPTDecoderLayerWeight<T>(
                embed_size_, hidden_units_, inter_size_, tensor_para_size_, tensor_para_rank_, flag ,int8_mode_, gpt_variant_params_));
        }
        else {
            // Don't malloc and load these layers since we don't use them.
            decoder_layer_weights.push_back(new SeedGPTDecoderLayerWeight<T>());
...
SeedGPTWeight<T>::~SeedGPTWeight()
{
    if (is_maintain_buffer == true) {
        ...
        pre_token_embedding.kernel    = nullptr;
        pre_token_embedding.bias      = nullptr;
        post_token_embedding.kernel   = nullptr;
        post_token_embedding.bias     = nullptr;
...
template<typename T>
SeedGPTWeight<T>::SeedGPTWeight(const SeedGPTWeight& other):
    embed_size_(other.embed_size_),
    ...
    shared_embed_(other.shared_embed_)
{
    ...
    cudaD2Dcpy(weights_ptr[1], other.weights_ptr[1], vocab_size_ * embed_size_);//hidden_units_
    ...
    if (gpt_variant_params_.has_post_decoder_layernorm) {
        cudaD2Dcpy(weights_ptr[4], other.weights_ptr[4], embed_size_);//hidden_units_
        cudaD2Dcpy(weights_ptr[5], other.weights_ptr[5], embed_size_);//hidden_units_
    }
    cudaD2Dcpy(weights_ptr[6], other.weights_ptr[6], embed_size_ * vocab_size_);
    cudaD2Dcpy(weights_ptr[7], other.weights_ptr[7], embed_size_ * hidden_units_);
    cudaD2Dcpy(weights_ptr[8], other.weights_ptr[8], hidden_units_);
    cudaD2Dcpy(weights_ptr[9], other.weights_ptr[9], embed_size_ * hidden_units_);
...
template<typename T>
SeedGPTWeight<T>& SeedGPTWeight<T>::operator=(const SeedGPTWeight& other)
{
    embed_size_                 = other.embed_size_;
    hidden_units_               = other.hidden_units_;
    inter_size_                 = other.inter_size_;
    ...
    cudaD2Dcpy(weights_ptr[6], other.weights_ptr[6], embed_size_ * vocab_size_); //hidden_units_
...
template<typename T>
void SeedGPTWeight<T>::setWeightPtr()
{
    ...
    pre_token_embedding.kernel = weights_ptr[7];
    pre_token_embedding.bias = weights_ptr[8];
    post_token_embedding.kernel = weights_ptr[9];
    post_token_embedding.bias = weights_ptr[10];
...
template<typename T>
void SeedGPTWeight<T>::mallocWeights()
{
    ...
    if (gpt_variant_params_.has_post_decoder_layernorm) {
        deviceMalloc(&weights_ptr[4], embed_size_);//hidden_units_
        deviceMalloc(&weights_ptr[5], embed_size_);//hidden_units_
    }
    // deceder embedding table.
    deviceMalloc(&weights_ptr[6], embed_size_ * vocab_size_);//hidden_units_
    deviceMalloc(&weights_ptr[7], embed_size_ * hidden_units_);
    deviceMalloc(&weights_ptr[8], hidden_units_);
    deviceMalloc(&weights_ptr[9], embed_size_ * hidden_units_);
...
template<typename T>
void SeedGPTWeight<T>::loadModel(std::string dir_path)
{
    FtCudaDataType model_file_type = getModelFileType(dir_path + "/config.ini", "seed_13B");
    FT_CHECK(is_maintain_buffer == true);
    if (gpt_variant_params_.has_positional_encoding) {
        loadWeightFromBin<T>(
            weights_ptr[0], {max_seq_len_, embed_size_}, dir_path + "/model.wpe.bin", model_file_type); //hidden_units_
    }
    loadWeightFromBin<T>(weights_ptr[1], {vocab_size_ * embed_size_}, dir_path + "/model.wte.bin", model_file_type);//hidden_units_
    if (gpt_variant_params_.has_pre_decoder_layernorm) {
        loadWeightFromBin<T>(
    ...
    if (gpt_variant_params_.has_post_decoder_layernorm) {
        loadWeightFromBin<T>(
            weights_ptr[4], {embed_size_}, dir_path + "/model.final_layernorm.bias.bin", model_file_type);//hidden_units_
        loadWeightFromBin<T>(
            weights_ptr[5], {embed_size_}, dir_path + "/model.final_layernorm.weight.bin", model_file_type);//hidden_units_
    }
    if (checkIfFileExist(dir_path + "/model.lm_head.weight.bin")) {
        shared_embed_ = false;
        loadWeightFromBin<T>(
            weights_ptr[6], {vocab_size_ * embed_size_}, dir_path + "/model.lm_head.weight.bin", model_file_type);
    }
    else {
        shared_embed_ = true;
        loadWeightFromBin<T>(
            weights_ptr[6], {vocab_size_ * embed_size_}, dir_path + "/model.wte.bin", model_file_type); //hidden_units_
    }
    if (checkIfFileExist(dir_path + "/model.pre_token_proj.weight.bin")) {
        loadWeightFromBin<T>(
            weights_ptr[7], {embed_size_ * hidden_units_}, dir_path + "/model.pre_token_proj.weight.bin", model_file_type);
        loadWeightFromBin<T>(
            weights_ptr[8], {hidden_units_}, dir_path + "/model.pre_token_proj.bias.bin", model_file_type);
    }
    if (checkIfFileExist(dir_path + "/model.post_token_proj.weight.bin")) {
        loadWeightFromBin<T>(
            weights_ptr[9], {embed_size_ * hidden_units_}, dir_path + "/model.post_token_proj.weight.bin", model_file_type);
        loadWeightFromBin<T>(
            weights_ptr[10], {embed_size_}, dir_path + "/model.post_token_proj.bias.bin", model_file_type);
    }
...
template<typename T>
bool SeedGPTWeight<T>::isValidLayerParallelId(int l)
{
...
template<typename T>
void SeedGPTWeight<T>::resizeLayer(const int num_layer, const int int8_mode)
{
    int8_mode_ = int8_mode;
    num_layer_ = num_layer;
    decoder_layer_weights.reserve(num_layer_);
    for (int l = 0; l < num_layer_; l++) {
        decoder_layer_weights.push_back(new SeedGPTDecoderLayerWeight<T>(int8_mode_));
    }
}

#ifdef SPARSITY_ENABLED
template<typename T>
void SeedGPTWeight<T>::compress_weights(cublasMMWrapper& cublas_wrapper)
{
...
template struct SeedGPTWeight<float>;
template struct SeedGPTWeight<half>;
#ifdef ENABLE_BF16
template struct SeedGPTWeight<__nv_bfloat16>;
#endif
