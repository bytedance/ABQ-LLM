...
#include "src/fastertransformer/models/seedgpt/SeedGPTDecoderLayerWeight.h"
...
template<typename T>
SeedGPTDecoderLayerWeight<T>::SeedGPTDecoderLayerWeight(const int        embed_size,
                                                        const int        hidden_units,
                                                        const int        inter_size,
                                                        const int        tensor_para_size,
                                                        const int        tensor_para_rank,
                                                        const bool       use_moe_lora,
                                                        const int        int8_mode,
...
template<typename T>
SeedGPTDecoderLayerWeight<T>::SeedGPTDecoderLayerWeight(const int int8_mode): int8_mode_(int8_mode)
{
}

template<typename T>
SeedGPTDecoderLayerWeight<T>::~SeedGPTDecoderLayerWeight()
{
    if (is_maintain_buffer == true) {
        ...
        ffn_weights.inter_lora_weight.kernel   = nullptr;
        ffn_weights.inter_lora_weight.bias     = nullptr;
        ffn_weights.output_weight.kernel       = nullptr;
        ffn_weights.output_weight.bias         = nullptr;
        ffn_weights.output_lora_weight.kernel  = nullptr;
        ffn_weights.output_lora_weight.bias    = nullptr;
...
template<typename T>
void SeedGPTDecoderLayerWeight<T>::copyFrom(const SeedGPTDecoderLayerWeight& other)
{
    ...
    if (int8_mode_ == 0) {
        ...
        cudaD2Dcpy(weights_ptr[20], other.weights_ptr[20], hidden_units_ * inter_size_ / tensor_para_size_);
        cudaD2Dcpy(weights_ptr[22], other.weights_ptr[26], inter_size_ / tensor_para_size_ * hidden_units_);
        if (use_moe_lora_) {
            cudaD2Dcpy(weights_ptr[21], other.weights_ptr[21], inter_size_ / tensor_para_size_);
            cudaD2Dcpy(weights_ptr[23], other.weights_ptr[27], hidden_units_);
        }
...
template<typename T>
SeedGPTDecoderLayerWeight<T>::SeedGPTDecoderLayerWeight(const SeedGPTDecoderLayerWeight& other):
    embed_size_(other.embed_size_),
    hidden_units_(other.hidden_units_),
    inter_size_(other.inter_size_),
    tensor_para_size_(other.tensor_para_size_),
    tensor_para_rank_(other.tensor_para_rank_),
    use_moe_lora_(other.use_moe_lora_),
...
template<typename T>
SeedGPTDecoderLayerWeight<T>& SeedGPTDecoderLayerWeight<T>::operator=(const SeedGPTDecoderLayerWeight& other)
{
    embed_size_         = other.embed_size_;
    hidden_units_       = other.hidden_units_;
    inter_size_         = other.inter_size_;
...
template<typename T>
void SeedGPTDecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
    ...
    loadWeightFromBin<T>(weights_ptr[3],
                         {3, hidden_units_ / tensor_para_size_},  // hidden_units_
    ...
    if (use_moe_lora_) {
        loadWeightFromBin<T>(weights_ptr[21],
                             {inter_size_ / tensor_para_size_},
                             dir_path + ".mlp.lora_dense_h_to_4h.bias." + std::to_string(tensor_para_rank_) + ".bin",
                             model_file_type);
        loadWeightFromBin<T>(
            weights_ptr[23], {hidden_units_}, dir_path + ".mlp.lora_dense_4h_to_h.bias.bin", model_file_type);
    }
    ...
    if (int8_mode_ == 0) {
        loadWeightFromBin<T>(weights_ptr[2],
                             {hidden_units_, 3 * hidden_units_ / tensor_para_size_},  // hidden_units_
        ...
        if (use_moe_lora_) {
            loadWeightFromBin<T>(weights_ptr[20],
                                 {hidden_units_, inter_size_ / tensor_para_size_},
                                 dir_path + ".mlp.lora_dense_h_to_4h.weight." + std::to_string(tensor_para_rank_)
                                     + ".bin",
                                 model_file_type);
            loadWeightFromBin<T>(weights_ptr[22],
                                 {inter_size_ / tensor_para_size_, hidden_units_},
                                 dir_path + ".mlp.lora_dense_4h_to_h.weight." + std::to_string(tensor_para_rank_)
                                     + ".bin",
                                 model_file_type);
        }
...
template<typename T>
void SeedGPTDecoderLayerWeight<T>::setWeightPtr()
{
    ...
    ffn_weights.inter_lora_weight.kernel   = weights_ptr[20];
    ffn_weights.inter_lora_weight.bias     = weights_ptr[21];
    ffn_weights.output_weight.kernel       = weights_ptr[10];
    ffn_weights.output_weight.bias         = weights_ptr[11];
    if (use_moe_lora_) {
        ffn_weights.output_lora_weight.kernel = weights_ptr[22];
        ffn_weights.output_lora_weight.bias   = weights_ptr[23];
    }
...
template<typename T>
void SeedGPTDecoderLayerWeight<T>::mallocWeights()
{
    ...
    deviceMalloc(&weights_ptr[21], inter_size_ / tensor_para_size_);       // ffn lora inter bias
    deviceMalloc(&weights_ptr[23], hidden_units_);                         // ffn lora output bias
    ...
    if (int8_mode_ == 0) {
        ...
        if (use_moe_lora_) {
            deviceMalloc(&weights_ptr[20], hidden_units_ * inter_size_ / tensor_para_size_);  // ffn lora inter weight
            deviceMalloc(&weights_ptr[22], inter_size_ / tensor_para_size_ * hidden_units_);  // ffn lora output weight
        }
...
template<typename T>
void SeedGPTDecoderLayerWeight<T>::compress_weights(cublasMMWrapper& cublas_wrapper, int hidden_dim)
{
...
template<typename T>
void SeedGPTDecoderLayerWeight<T>::transposeWeight()
{
...
template<typename T>
void SeedGPTDecoderLayerWeight<T>::transposeCalibrateQuantizeWeight()
{
...
template struct SeedGPTDecoderLayerWeight<float>;
template struct SeedGPTDecoderLayerWeight<half>;
#ifdef ENABLE_BF16
template struct SeedGPTDecoderLayerWeight<__nv_bfloat16>;
#endif
