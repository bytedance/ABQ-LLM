...
inline __device__ float2 rotary_embedding_coefficient(
    const int zid, const int rot_embed_dim, const float t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    const float t_scale  = t_step / rope_scaling;
    const float inv_freq = t_scale / pow(base, zid / (float)rot_embed_dim);  // rope ** ( ( 0 -- 128 step==2) / 128)
...
inline __device__ void apply_rotary_embedding(
    float& q, int zid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    return;
}

inline __device__ void apply_rotary_embedding(
    float& q, float& k, int zid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    return;
}

inline __device__ void apply_rotary_embedding(
    float2& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (2 * tid >= rot_embed_dim) {
        return;
    }
    const auto coef = rotary_embedding_coefficient(2 * tid, rot_embed_dim, t_step, base, rope_scaling);
    q               = rotary_embedding_transform(q, coef);
}

inline __device__ void apply_rotary_embedding(
    float2& q, float2& k, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (2 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    float4& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    ...
    const auto coef0 = rotary_embedding_coefficient(4 * tid, rot_embed_dim, t_step, base, rope_scaling);
    q_.x             = rotary_embedding_transform(q_.x, coef0);
    const auto coef1 = rotary_embedding_coefficient(4 * tid + 2, rot_embed_dim, t_step, base, rope_scaling);
    q_.y             = rotary_embedding_transform(q_.y, coef1);
}

inline __device__ void apply_rotary_embedding(
    float4& q, float4& k, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (4 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    uint32_t& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (2 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    uint32_t& q, uint32_t& k, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (2 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    uint2& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (4 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    uint2& q, uint2& k, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (4 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    uint4& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (8 * tid >= rot_embed_dim) {
        return;
    }
    const auto coef0 = rotary_embedding_coefficient(8 * tid, rot_embed_dim, t_step, base, rope_scaling);
    q.x              = rotary_embedding_transform(q.x, coef0);
    const auto coef1 = rotary_embedding_coefficient(8 * tid + 2, rot_embed_dim, t_step, base, rope_scaling);
    q.y              = rotary_embedding_transform(q.y, coef1);
    const auto coef2 = rotary_embedding_coefficient(8 * tid + 4, rot_embed_dim, t_step, base, rope_scaling);
    q.z              = rotary_embedding_transform(q.z, coef2);
    const auto coef3 = rotary_embedding_coefficient(8 * tid + 6, rot_embed_dim, t_step, base, rope_scaling);
    q.w              = rotary_embedding_transform(q.w, coef3);
}

inline __device__ void apply_rotary_embedding(
    uint4& q, uint4& k, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (8 * tid >= rot_embed_dim) {
        return;
    }
...
#ifdef ENABLE_BF16
inline __device__ void apply_rotary_embedding(
    __nv_bfloat162& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (2 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    bf16_4_t& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (4 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    bf16_4_t& q, bf16_4_t& k, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (4 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    bf16_8_t& q, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (8 * tid >= rot_embed_dim) {
        return;
    }
...
inline __device__ void apply_rotary_embedding(
    bf16_8_t& q, bf16_8_t& k, int tid, int rot_embed_dim, int t_step, float base = 10000.0f, float rope_scaling = 1.0f)
{
    if (8 * tid >= rot_embed_dim) {
        return;
    }
