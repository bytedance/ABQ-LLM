...
template<typename T>
TensorParallelLlamaV2DecoderSelfAttentionLayer<T>::TensorParallelLlamaV2DecoderSelfAttentionLayer(
...
TensorParallelLlamaV2DecoderSelfAttentionLayer<T>::TensorParallelLlamaV2DecoderSelfAttentionLayer(
    ...
    std::shared_ptr<AbstractCustomComm> custom_all_reduce_comm,
    int                                 enable_custom_all_reduce):
    TensorParallelLlamaV2DecoderSelfAttentionLayer(max_batch_size,
                                                   head_num,
                                                   kv_head_num,
                                                   size_per_head,
                                                   0,
...
template<typename T>
TensorParallelLlamaV2DecoderSelfAttentionLayer<T>::TensorParallelLlamaV2DecoderSelfAttentionLayer(
    TensorParallelLlamaV2DecoderSelfAttentionLayer<T> const& attention_layer):
    LlamaV2DecoderSelfAttentionLayer<T>(attention_layer),
...
template<typename T>
void TensorParallelLlamaV2DecoderSelfAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                                                ...
                                                                const AttentionWeight<T>* attention_weights)
{
    ...
    LlamaV2DecoderSelfAttentionLayer<T>::forward(output_tensors, input_tensors, attention_weights);
    ...
    if (tensor_para_.world_size_ > 1 && do_all_reduce_) {
        if (!use_custom_all_reduce_kernel) {
            ftNcclAllReduceSum(
                attention_out, attention_out, size, tensor_para_, LlamaV2DecoderSelfAttentionLayer<T>::stream_);
        }
        else {
            custom_all_reduce_comm_->customAllReduce(size, LlamaV2DecoderSelfAttentionLayer<T>::stream_);
        }
...
template class TensorParallelLlamaV2DecoderSelfAttentionLayer<float>;
template class TensorParallelLlamaV2DecoderSelfAttentionLayer<half>;
#ifdef ENABLE_BF16
template class TensorParallelLlamaV2DecoderSelfAttentionLayer<__nv_bfloat16>;
#endif
