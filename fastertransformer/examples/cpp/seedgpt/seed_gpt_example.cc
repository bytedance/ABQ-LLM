...
template<typename T>
void seed_gpt_example(const INIReader reader, std::string in_csv);
...
int main(int argc, char* argv[])
{
    ...
    if (argc >= 2) {
        ...
        ini_name = "../examples/cpp/seedgpt/gpt_config.ini";
    }
    ...
    if (argc == 3) {
        ...
        in_csv = "../examples/cpp/seedgpt/start_ids.csv";
    }
    ...
    if (reader.GetBoolean("request", "receive_input_ids", false)) {
        in_csv = "../examples/cpp/seedgpt/start_ids.csv";
    }

    if (data_type == "fp32") {
        seed_gpt_example<float>(reader, in_csv);
    }
    else if (data_type == "fp16") {
        seed_gpt_example<half>(reader, in_csv);
    }
#ifdef ENABLE_BF16
    else if (data_type == "bf16") {
        seed_gpt_example<__nv_bfloat16>(reader, in_csv);
    }
#endif
    else {
...
void seed_gpt_example(const INIReader reader, std::string in_csv)
{
    ...
    SeedGPTWeight<T> gpt_weights(model_config.embed_size,
                                     model_config.hidden_units,
                                     model_config.inter_size,
                                     model_config.vocab_size,
                                     model_config.decoder_layers,
                                     ...
                                     pipeline_para.rank_,
                                     request_config.lora_r == -1,
                                     request_config.lora_start_layer,
    ...
    AttentionType attention_type = getAttentionType<T>(model_config.size_per_head,
                                                       ...
                                                       true);                        // causal_mask
    SeedGPT<T> gpt = SeedGPT<T>(0,  // max_batch_size, FT will adjust the buffer automatically.
    ...
    T*                                      d_input_embedding     = nullptr;
    int*                                    d_input_lora_mask     = nullptr;
    ...
    if (request_config.receive_input_ids) {
        ...
        populate_request_embedding(input_tensors,
                                   output_tensors,
                                   d_input_embedding,
                                   d_input_lora_mask,
                                   d_input_lengths,
...
//        gpt.forward(&output_tensors, &input_tensors, &gpt_weights);
    ...
    safe_free(d_input_ids); //d_input_embedding
