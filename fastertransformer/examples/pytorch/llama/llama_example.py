...
from examples.pytorch.llama.llama import LLAMA, LLAMAWeights
...
def main():
    ...
    if 'llama' in ckpt_config.keys():
        for args_key, config_key, func in [
            ...
            if config_key in ckpt_config['llama'].keys():
                prev_val = args.__dict__[args_key]
                args.__dict__[args_key] = func('llama', config_key)
        ...
        for key in ['head_num', 'size_per_head', 'tensor_para_size']:
            if key in args.__dict__:
                ...
                args.__dict__[key] = ckpt_config.getint('llama', key)
                print('Loading {} from config.ini,    previous: {},    current: {}'.format(
                    key, prev_val, args.__dict__[key]))
            else:
                print('Not loading {} from config.ini'.format(key))
    if 'structure' in ckpt_config.keys():
        llama_with_moe = ckpt_config.getboolean('structure', 'llama_with_moe')
    ...
    if args.tokenizer_dir and os.path.exists(args.tokenizer_dir):
        enc = AutoTokenizer.from_pretrained(args.tokenizer_dir, use_fast=False)
        enc.pad_token_id = enc.bos_token_id
    elif args.use_jieba_tokenizer:
    ...
    if args.tokenizer_dir and os.path.exists(args.tokenizer_dir):
        if args.sample_input_file:  # conditional case
            ...
            start_ids = enc(contexts, 
                            return_tensors='pt', 
                            padding="longest").input_ids
            max_seq_len = start_ids.shape[1]
            start_lengths = [max_seq_len for ids in range(start_ids.shape[0])]
            start_lengths = torch.IntTensor(start_lengths)        
        else:  # unconditional case
            batch_size = max_batch_size
            contexts = ['<|endoftext|>'] * batch_size
            start_ids = torch.concat([torch.IntTensor([end_id])] * batch_size, dim=0)
            start_lengths = [1 for ids in range(batch_size)]
            start_lengths = torch.IntTensor(start_lengths) 
    else:
        if args.sample_input_file:  # conditional case
            with open(args.sample_input_file, "r") as f:
    ...
    if args.inference_data_type == 'fp8':
        llama = LLAMAFP8(head_num, size_per_head, vocab_size, start_id, end_id, layer_num,
        ...
        llama = LLAMA(head_num, size_per_head, vocab_size, start_id, end_id, layer_num,
              max_seq_len, tensor_para_size, pipeline_para_size, lib_path=args.lib_path,
              inference_data_type=args.inference_data_type,
              weights_data_type=args.weights_data_type,
              llama_with_moe=llama_with_moe,
              expert_num=expert_num,
              moe_k=moe_k,
              moe_layer_index=moe_layer_index)
    if not llama.load(ckpt_path=args.ckpt_path):
