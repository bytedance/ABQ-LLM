   ...
   bool   is_return_context_cum_log_probs)
{
   ...
   FT_CHECK_WITH_INFO(!is_return_context_cum_log_probs, "llama does not currently support is_return_context_cum_log_probs!");
...
void Llama<T>::freeBuffer()
{
   if (is_allocate_buffer_) {
       ...
       if(context_decoder_normed_input_buf_ != nullptr){
            allocator_->free((void**)(&context_decoder_normed_input_buf_));
       }
       if(decoder_normed_input_buf_ != nullptr){
...
Llama<T>::Llama(Llama<T> const& gpt):
   ...
   enable_flash_attn_(gpt.enable_flash_attn_)
{
   initialize();
}
...
{
   FT_CHECK_WITH_INFO(false, "llama does not support vector<Tensor> forward api !");
}
                      ...
                      const LlamaWeight<T>*                        gpt_weights)
{
   ...
   // handle first step
   // input_tensors->at("input_ids") --> tiled_input_ids_buf_
   // [batch, max_input_length] --> [batch*beam_width, max_input_length]
   if (has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1) {
       ...
       if (has_prefix_soft_prompt_) {
           ...
           max_input_length += max_prefix_soft_prompt_length;  // view soft_prompt as input
            ...
            // context_decoder_input_buf_: [batch*beam_width, max_input_length] --> [batch*beam_width, max_input_length, hidden_units_]
            // output_ids_buf_:  [batch*beam_width, max_input_length] --> [batch*beam_width, max_input_length]
       ...
       // input_attention_mask_: [batch*beam_width, max_input_length] --> [batch*beam_width, 1, max_input_length, max_input_length+max_prefix_prompt_length]
       PUSH_RANGE("build decoder attention mask");
       invokeBuildDecoderAttentionMask(input_attention_mask_,
                                       tiled_input_lengths_buf_,
                                       tiled_prompt_lengths_buf_,       // nullptr
                                       batch_size * beam_width,
                                       max_input_length,
                                       max_prefix_prompt_length,        // 0
                                       stream_);
       sync_check_cuda_error();
       POP_RANGE;

//       T *ctx = new T [hidden_units_];
//       cudaD2Hcpy(ctx, context_decoder_input_buf_, hidden_units_);
//       printPerLine((half*)ctx, hidden_units_, "emb:");
//       cudaD2Hcpy(ctx, context_decoder_input_buf_ + hidden_units_ * (max_input_length-1), hidden_units_);
//       printPerLine((half*)ctx, hidden_units_, "emb(-1):");
       ...
       std::unordered_map<std::string, Tensor> decoder_output_tensors{
        ...
        // {batch_size * beam_width, (size_t)max_input_length, hidden_units_} --> 
        // {batch_size * beam_width, (size_t)max_input_length, hidden_units_}
       PUSH_RANGE("gpt_context_decoder start");
       gpt_context_decoder_->forward(
           &decoder_output_tensors, &decoder_input_tensors, &gpt_weights->decoder_layer_weights);
       sync_check_cuda_error();
       POP_RANGE;

        // finished_buf_: [batch_size*beam_width] filled false 
        // sequence_lengths_: [batch_size*beam_width] filled max_input_length - 1 
        // cum_log_probs: [batch_size*beam_width] filled 0 or (T)-MAX_T_VAL
   ...
   // masked_tokens_[batch*beam, max_cache_seq_len], [:, max_input_length:max_cache_seq_len] = True
...
//               T *norm = new T [hidden_units_];
//               cudaD2Hcpy(norm, decoder_output_buf_, hidden_units_);
//               printPerLine((half*)norm, hidden_units_, "ctx out:", step);
                // {batch_size * beam_width,  hidden_units_} --> 
               ...
               if (tensor_para_.world_size_ == 1) {
                   ...
                   cublas_wrapper_->Gemm(CUBLAS_OP_T,
                    ...
                    // {batch_size * beam_width,  hidden_units_} x {local_vocab_size, hidden_units_}--> 
                    // {batch_size * beam_width,  local_vocab_size} x TP
