...
template<typename T>
void LlamaV2<T>::initialize()
{
    gpt_context_decoder_ = new LlamaV2ContextDecoder<T>(head_num_,
                                                        kv_head_num_,
                                                        size_per_head_,
                                                        inter_size_,
                                                        num_layer_,
    ...
    gpt_decoder_ = new LlamaV2Decoder<T>(head_num_,
                                         kv_head_num_,
                                         size_per_head_,
                                         inter_size_,
                                         num_layer_,
...
template<typename T>
LlamaV2<T>::LlamaV2(LlamaV2<T> const& gpt):
    BaseLayer(gpt),
    ...
    rope_scaling_(gpt.rope_scaling_),
...
template<typename T>
void LlamaV2<T>::registerCallback(callback_sig* fn, void* ctx)
{
...
template<typename T>
void LlamaV2<T>::unRegisterCallback()
{
...
template<typename T>
void LlamaV2<T>::forward(std::vector<Tensor>*       output_tensors,
                         const std::vector<Tensor>* input_tensors,
                         const LlamaV2Weight<T>*    gpt_weights)
{
    FT_CHECK(false);
}

template<typename T>
void LlamaV2<T>::forward(std::unordered_map<std::string, Tensor>*       output_tensors,
...
template<typename T>
void LlamaV2<T>::sendTensorsToFirstPipelineNode(std::unordered_map<std::string, Tensor>*       output_tensors,
                                                const std::unordered_map<std::string, Tensor>* input_tensors)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (pipeline_para_.world_size_ == 1) {
...
template<typename T>
void LlamaV2<T>::setOutputTensors(std::unordered_map<std::string, Tensor>*       output_tensors,
                                  const std::unordered_map<std::string, Tensor>* input_tensors,
...
template<typename T>
size_t LlamaV2<T>::getPipelineParallelRank()
{
    return pipeline_para_.rank_;
}

template<typename T>
size_t LlamaV2<T>::getPipelineParallelSize()
{
    return pipeline_para_.world_size_;
}

template<typename T>
size_t LlamaV2<T>::getTensorParallelRank()
{
    return tensor_para_.rank_;
}

template<typename T>
size_t LlamaV2<T>::getTensorParallelSize()
{
    return tensor_para_.world_size_;
}

template<typename T>
bool* LlamaV2<T>::getFinishBuffer()
{
