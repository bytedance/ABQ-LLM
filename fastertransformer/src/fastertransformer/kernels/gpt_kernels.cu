...
#pragma unroll 1
        ...
        T pos_embed        = pos_table == nullptr ? (T)0.f : pos_table[(step - 1) * hidden_units + col_index]; // len_id x hid + hid_id
...
template<typename T>
__global__ void combined_text_image_embeding(T* text_embed, T* image_embed, const T* pos_table, int sys_len,
                                             int text_len,int image_len, int embed_sz,  int batch_size) {

    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < batch_size * image_len * embed_sz;
         idx += blockDim.x * gridDim.x) {
        int index = idx % embed_sz;
        int img_index = idx / embed_sz;
        int w_index_row = img_index / image_len; // b
        int w_index_col = img_index % image_len;

        text_embed[w_index_row * (sys_len + image_len) * embed_sz + (sys_len + w_index_col) * embed_sz + index] =
            image_embed[w_index_row * image_len * embed_sz + w_index_col * embed_sz + index] + pos_table[(sys_len + w_index_col) * embed_sz + index];
    }

}

template<typename T>
void invokeCombinedTextAndImageEmbedding(T* text_embed,
                                         T* image_embed,
                                         const T* pos_table,
                                         int sys_len,
                                         int text_len,
                                         ...
                                         int batch_size,
                                         cudaStream_t stream) {
//    cudaD2Dcpy(text_embed + (image_len + sys_len) * embed_size, text_embed + sys_len * embed_size, embed_size * 21);
    dim3 gridSize(min(batch_size * image_len, 65536));
    dim3 blockSize(min(512, embed_size));
    combined_text_image_embeding<<<gridSize, blockSize, 0, stream>>>(text_embed, image_embed, pos_table, sys_len, text_len, image_len, embed_size, batch_size);
}

#define INSTANTIATE_INVOKE_TEXT_IMAGE_EMBED(T)                                                                 \
    template void invokeCombinedTextAndImageEmbedding(T* text_embed,                                           \
                                                      T* image_embed,                                          \
                                                      const T* pos_table,                                      \
                                                      int sys_len,                                             \
                                                      int text_len,                                            \
...
INSTANTIATE_INVOKE_TEXT_IMAGE_EMBED(half);
INSTANTIATE_INVOKE_TEXT_IMAGE_EMBED(float);
#ifdef ENABLE_BF16
INSTANTIATE_INVOKE_TEXT_IMAGE_EMBED(__nv_bfloat16);
#endif
#undef INSTANTIATE_INVOKE_TEXT_IMAGE_EMBED
