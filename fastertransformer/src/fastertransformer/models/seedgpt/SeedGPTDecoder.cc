...
#include "src/fastertransformer/models/seedgpt/SeedGPTDecoder.h"
#include "src/fastertransformer/utils/nvtx_utils.h"

namespace fastertransformer {

template<typename T>
void SeedGPTDecoder<T>::initialize()
{
...
template<typename T>
SeedGPTDecoder<T>::SeedGPTDecoder(size_t                              max_batch_size,
                                          size_t                              head_num,
...
template<typename T>
SeedGPTDecoder<T>::SeedGPTDecoder(SeedGPTDecoder<T> const& decoder):
...
template<typename T>
void SeedGPTDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void SeedGPTDecoder<T>::allocateBuffer(size_t batch_size)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void SeedGPTDecoder<T>::freeBuffer()
{
...
template<typename T>
bool SeedGPTDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool SeedGPTDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool SeedGPTDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int SeedGPTDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
SeedGPTDecoder<T>::~SeedGPTDecoder()
{
...
template<typename T>
void SeedGPTDecoder<T>::forward(std::unordered_map<std::string, Tensor>*              output_tensors,
...
template class SeedGPTDecoder<float>;
template class SeedGPTDecoder<half>;
#ifdef ENABLE_BF16
template class SeedGPTDecoder<__nv_bfloat16>;
#endif
