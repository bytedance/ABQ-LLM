...
template<typename T>
struct SeedGPTWeight {

    SeedGPTWeight() = default;
    SeedGPTWeight(const int                                  embed_size,
    ...
    SeedGPTWeight(const int                                  embed_size,
                  ...
                  const int                                  layer_para_size,
                  const int                                  layer_para_rank,
                  const bool                                 moe_lora_mode        = false,
                  const int                                  lora_start_layer     = 0,
                  const int                                  int8_mode            = 0,
                  PromptLearningType                         prompt_learning_type = PromptLearningType::no_prompt,
                  std::map<std::string, std::pair<int, int>> prompt_learning_pair = {},
                  gptVariantParams                           gpt_variant_params   = {});
    ~SeedGPTWeight();
    SeedGPTWeight(const SeedGPTWeight& other);
    SeedGPTWeight& operator=(const SeedGPTWeight& other);
    ...
    std::vector<SeedGPTDecoderLayerWeight<T>*> decoder_layer_weights;
    ...
    DenseWeight<T>                                 pre_token_embedding;
    DenseWeight<T>                                 post_token_embedding;
...
private:
    ...
    //  - 4 for the pre/post prj token.

    size_t num_base_weights = 11;
