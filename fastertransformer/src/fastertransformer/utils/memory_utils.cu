...
template void cudaH2Dcpy(unsigned short* tgt, const unsigned short* src, size_t size);
#ifdef ENABLE_BF16
                                                  ...
                                                  QuantType           quant_type)
{
    ...
    size_t        num_elts = shape[0] * shape[1];
    if(quant_type == QuantType::PACKED_INT4_WEIGHT_ONLY){
        FT_CHECK_WITH_INFO(shape[1] % 2 == 0, "Int4 quantization requires that the N dimension be a multiple of 2!");
        num_elts = num_elts / 2;
    } 
...
int loadWeightFromBinAndQuantizeForWeightOnly(int8_t*             quantized_weight_ptr,
                                              ...
                                              int                 int8_mode)
{
    FT_CHECK_WITH_INFO(int8_mode == 1 ||  int8_mode == 4, "We can only use this function to W4 and W8.");
    auto quant_type = int8_mode == 1 ? QuantType::INT8_WEIGHT_ONLY : QuantType::PACKED_INT4_WEIGHT_ONLY ;
    switch (model_file_type) {
        case FtCudaDataType::FP32:
            loadWeightFromBinAndQuantizeForWeightOnlyFunc<T, float>(quantized_weight_ptr, scale_ptr, shape, filename, quant_type);
            break;
        case FtCudaDataType::FP16:
            loadWeightFromBinAndQuantizeForWeightOnlyFunc<T, half>(quantized_weight_ptr, scale_ptr, shape, filename, quant_type);
            break;
#ifdef ENABLE_BF16
        case FtCudaDataType::BF16:
...
template int
loadWeightFromBinAndQuantizeForWeightOnly(int8_t*, float*, std::vector<size_t>, std::string, FtCudaDataType, int);
template int
loadWeightFromBinAndQuantizeForWeightOnly(int8_t*, half*, std::vector<size_t>, std::string, FtCudaDataType, int);
#ifdef ENABLE_BF16
template int
loadWeightFromBinAndQuantizeForWeightOnly(int8_t*, __nv_bfloat16*, std::vector<size_t>, std::string, FtCudaDataType, int);
#endif
