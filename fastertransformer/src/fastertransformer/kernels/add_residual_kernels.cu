...
template<typename T, int RESIDUAL_NUM, typename T2 = T>
__global__ void addLoraMaskBiasResidual(T*           output,
                                        const T2*    input,
                                        const T2*    lora_input,
                                        const int*   mask,
                                        const T*     residual1,
                                        ...
                                        const int    n)
{
    ...
    if (col_index < n) {
        T bias_val = (bias == nullptr) ? (T)(0.0f) : bias[col_index];
        T lora_bias_val = (lora_bias == nullptr) ? (T)(0.0f) : bias[col_index];
        T in;
        if (std::is_same<T, T2>::value) {
            if (mask[blockIdx.x] == 0) {
                in = cuda_cast<T>(input[blockIdx.x * n + col_index]) + bias_val;  // cast required for compilation when T != T2
            } else {
                in = cuda_cast<T>(lora_input[blockIdx.x * n + col_index]) + lora_bias_val;  // cast required for compilation when T != T2
            }
        }
        else {
            in = cuda_cast<float>(input[blockIdx.x * n + col_index]) * (*scale_inter) * (*scale_out);
        ...
        if (RESIDUAL_NUM == 1) {
            output[blockIdx.x * n + col_index] = in + residual1[blockIdx.x * n + col_index];
        }
        else if (RESIDUAL_NUM == 2) {
            output[blockIdx.x * n + col_index] =
                in + residual1[blockIdx.x * n + col_index] + residual2[blockIdx.x * n + col_index];
        }
    }
}
...
template<typename T>
void invokeLoraMaskAddBiasResidual(T*           output,
                                   const T*     input,
                                   const T*     lora_input,
                                   const int*   mask,
                                   const T*     residual1,
                                   ...
                                   cudaStream_t stream)
{
    ...
    if (residual2 == nullptr) {
        if (should_scale_input) {
            addLoraMaskBiasResidual<T, 1><<<grid, block, 0, stream>>>(output,
                                                              reinterpret_cast<const int32_t*>(input),
                                                                      reinterpret_cast<const int32_t*>(lora_input),
                                                                      mask,
                                                              residual1,
                                                              residual2,
                                                              bias,
            ...
            addLoraMaskBiasResidual<T, 1>
                <<<grid, block, 0, stream>>>(output, input, lora_input, mask,residual1, residual2, bias, lora_bias, nullptr, nullptr, m, n);
        }
    }
    else {
        if (should_scale_input) {
            addLoraMaskBiasResidual<T, 2><<<grid, block, 0, stream>>>(output,
                                                              reinterpret_cast<const int32_t*>(input),
                                                                      reinterpret_cast<const int32_t*>(lora_input),
                                                                      mask,
                                                              residual1,
...
template<typename T>
void invokeLoraMaskAddBiasResidual(
    T* output, const T* lora_input, const int* mask, const T* residual1, const T* residual2, const T* bias, const T* lora_bias, const int m, const int n, cudaStream_t stream)
{
    invokeLoraMaskAddBiasResidual(output, output, lora_input, mask, residual1, residual2, bias, lora_bias, nullptr, nullptr, m, n, stream);
}
...
#define INSTANTIATE_LORA_INVOKE_ADD_BIAS_RESIDUAL(T)                                                                           \
    template void invokeLoraMaskAddBiasResidual(T*           output,                                                           \
                                                const T*     input,                                                            \
                                                const T*     lora_input,                                                       \
                                                const int*   mask,                                                              \
                                                const T*     residual1,                                                        \
                                                ...
                                                const int    n,                                                                \
                                                cudaStream_t stream)
INSTANTIATE_LORA_INVOKE_ADD_BIAS_RESIDUAL(float);
INSTANTIATE_LORA_INVOKE_ADD_BIAS_RESIDUAL(half);
#ifdef ENABLE_BF16
INSTANTIATE_LORA_INVOKE_ADD_BIAS_RESIDUAL(__nv_bfloat16);
#endif
#undef INSTANTIATE_LORA_INVOKE_ADD_BIAS_RESIDUAL

template void invokeLoraMaskAddBiasResidual(float*       output,
                                            const float* lora_input,
                                            const int*   mask,
                                            const float* residual1,
                                            const float* residual2,
                                            const float* bias,
                                            const float* lora_bias,
                                            const int    m,
                                            const int    n,
                                            cudaStream_t stream);

template void invokeLoraMaskAddBiasResidual(half*        output,
                                            const half*  lora_input,
                                            const int*   mask,
                                            const half*  residual1,
                                            const half*  residual2,
                                            const half*  bias,
                                            const half*  lora_bias,
                                            const int    m,
                                            const int    n,
                                            cudaStream_t stream);

#ifdef ENABLE_BF16
template void invokeLoraMaskAddBiasResidual(__nv_bfloat16*       output,
                                            const __nv_bfloat16* lora_input,
                                            const int*           mask,
                                            const __nv_bfloat16* residual1,
                                            const __nv_bfloat16* residual2,
                                            const __nv_bfloat16* bias,
                                            const __nv_bfloat16* lora_bias,
                                            const int            m,
                                            const int            n,
                                            cudaStream_t         stream);
#endif
