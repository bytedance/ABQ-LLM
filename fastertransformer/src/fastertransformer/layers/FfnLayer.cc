    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1 || int8_mode_ == 4) {
            ...
            abq_gemm_wrapper_->gemm(m,
                                    inter_size_,
                                    hidden_units_,
                                    reinterpret_cast<const half*>(input_tensor),
    ...
    if (use_sparse_gemm) {
        ...
        if (int8_mode_ == 1 || int8_mode_ == 4) {
            ...
            abq_gemm_wrapper_->gemm(m,
                                    hidden_units_,
                                    inter_size_,
                                    reinterpret_cast<const half*>(inter_buf_),
    ...
    int8_fc_runner_(int8_mode == 2 ? std::make_shared<CutlassInt8GemmRunner<T>>() : nullptr)
{
    ...
    if (int8_mode_ == 0) {
        ...
        // w2a8
        abq_gemm_wrapper_ = std::make_shared<ABQGEMMWrapper>(8, 2, true);
    }
}
...
void FfnLayer<T>::allocateBuffer(size_t token_num, int moe_k, bool use_moe)
{
    ...
    if (use_moe) {
        ...
        if (int8_mode_ == 1 ) {
            ...
            const int max_size    = std::max(hidden_units_, inter_size_);
            abq_gemm_ws_bytes_   = 8 * token_num * max_size / (sizeof(char) * 8);
