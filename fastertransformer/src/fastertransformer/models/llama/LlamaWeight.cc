...
template<typename T>
void LlamaWeight<T>::resizeLayer(const int num_layer, const int int8_mode)
{
   int8_mode_ = int8_mode;
   num_layer_ = num_layer;
   decoder_layer_weights.reserve(num_layer_);
   for (int l = 0; l < num_layer_; l++) {
       decoder_layer_weights.push_back(new LlamaDecoderLayerWeight<T>(int8_mode_));
   }
}
...
template<typename T>
void LlamaWeight<T>::compress_weights(cublasMMWrapper& cublas_wrapper)
{
    ...
    for (size_t i = 0; i < num_layer_; i++)
    {
        ...
        {
#ifdef SPARSITY_ENABLED
            decoder_layer_weights[i]->compress_weight(cublas_wrapper, hidden_units_,
                false, i,  tensor_para_rank_);
...
template<typename T>
void LlamaWeight<T>::compress_weights(cublasMMWrapper& cublas_wrapper, const bool release_dense)
{
    FT_CHECK(decoder_layer_weights.size() == static_cast<size_t>(num_layer_));
    for (size_t i = 0; i < num_layer_; i++)
    {
...
#ifdef SPARSITY_ENABLED
            decoder_layer_weights[i]->compress_weight(cublas_wrapper, hidden_units_,
                release_dense, i, tensor_para_rank_);
#endif //SPARSITY_ENABLED
        }
