                                          ...
                                          const AttentionWeight<T>* attention_weights)
{
    ...
    if (use_sparse) {
        ...
        // fp16/bf16 -> fp16
        abq_gemm_wrapper_->gemm(m,
                                3 * local_hidden_units_,  // n
                                hidden_units_,            // k
                                reinterpret_cast<const half*>(attention_input),
...
GptContextAttentionLayer<T>::GptContextAttentionLayer(size_t           max_batch_size,
    ...
    abq_gemm_wrapper_(int8_mode == 5 ? std::make_shared<ABQGEMMWrapper>(8, 2, true) : nullptr),
...
GptContextAttentionLayer<T>::GptContextAttentionLayer(GptContextAttentionLayer<T> const& attention_layer):
    ...
    abq_gemm_wrapper_(attention_layer.abq_gemm_wrapper_),
...
void GptContextAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool allocate_qk_buf)
{
    ...
    if (int8_mode_ == 1) {
        ...
        const int max_size  = std::max(hidden_units_, 3 * local_hidden_units_);
        abq_gemm_ws_bytes_  = 8 * batch_size * seq_len * max_size / (sizeof(char) * 8);
        abq_gemm_workspace_ = (char*)allocator_->reMalloc(abq_gemm_workspace_, abq_gemm_ws_bytes_, false);
    }
...
void GptContextAttentionLayer<T>::freeBuffer()
{
    ...
    if (is_allocate_buffer_) {
        ...
        if (abq_gemm_workspace_) {
            allocator_->free((void**)(&abq_gemm_workspace_));
            abq_gemm_ws_bytes_ = 0;
        }
