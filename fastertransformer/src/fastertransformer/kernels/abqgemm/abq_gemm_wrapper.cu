...
#include "src/fastertransformer/kernels/abqgemm/abq_gemm_wrapper.h"
#include "src/fastertransformer/kernels/abqgemm/mma_any/aq_bmma_library.h"
...
__global__ void pack_half(const half* in_data, const float* scale, unsigned int* packed_data, const int M, const int K)
{
    const unsigned int bit         = blockIdx.y;
    const int          L           = M * (K / 32);
    const float        local_scale = scale[0];
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < L; idx += blockDim.x) {
        unsigned int pack_val = 0;
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            pack_val |= ((static_cast<int>(val.x[0] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 1 - i * 8);
            pack_val |= ((static_cast<int>(val.x[1] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 2 - i * 8);
            pack_val |= ((static_cast<int>(val.x[2] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 3 - i * 8);
            pack_val |= ((static_cast<int>(val.x[3] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 4 - i * 8);
            pack_val |= ((static_cast<int>(val.x[4] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 5 - i * 8);
            pack_val |= ((static_cast<int>(val.x[5] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 6 - i * 8);
            pack_val |= ((static_cast<int>(val.x[6] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 7 - i * 8);
            pack_val |= ((static_cast<int>(val.x[7] * __float2half(local_scale)) >> bit) & 0x1) << (32 - 8 - i * 8);
        }
        // printf("M = %d, K = %d, L = %d, bit = %d, pack_val = %x\n", M, K, L, bit, pack_val);
        packed_data[bit * L + idx] = pack_val;
    }
