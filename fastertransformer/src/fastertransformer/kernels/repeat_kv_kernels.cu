...
template<typename T>
__global__ void repeat_kv_kernel(T* dst, const T* src, const int kv_head_num, const int repeat_num, const int size_per_head, const int token_num)
{
...
template<typename T>
void invokeKvRepeat(T* dst, const T* src, const int head_num, const int kv_head_num, const int size_per_head, const int token_num, cudaStream_t stream)
{
   dim3      block, grid;
   ...
   repeat_kv_kernel<T><<<grid, block, 0, stream>>>(dst, src, kv_head_num, head_num/kv_head_num, size_per_head, token_num);
}

template void
invokeKvRepeat(float* dst, const float* src, const int head_num, const int kv_head_num, const int size_per_head, const int token_num, cudaStream_t stream);
template void
invokeKvRepeat(half* dst, const half* src, const int head_num, const int kv_head_num, const int size_per_head, const int token_num, cudaStream_t stream);
#ifdef ENABLE_BF16
template void invokeKvRepeat(__nv_bfloat16* dst,
