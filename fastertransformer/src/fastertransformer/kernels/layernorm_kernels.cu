    ...
    */
    ...
    if (dynamic_quant) {
        ...
        generalAddResidualT5LayerNorm<T, true>
            <<<grid, block, 0, stream>>>(input, gamma, output, norm_output, layernorm_eps, m, n, scale, dynamic_scale, int8_mode);
    }else{
                                                ...
                                                cudaStream_t stream)
{
    if (beta != nullptr) {
        ...
        invokeGeneralAddResidualT5PreLayerNorm(output, norm_output, input, gamma, layernorm_eps, m, n, scale, dynamic_scale, int8_mode, stream);
    }
    return;
}
