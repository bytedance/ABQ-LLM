...
LlamaTritonModel<T>::LlamaTritonModel(size_t      tensor_para_size,
   ...
   shared_weights_(std::vector<std::shared_ptr<ft::LlamaV2Weight<T>>>(ft::getDeviceCount())),
   ...
   std::shared_ptr<ft::AbstractCustomComm>                           custom_all_reduce_comm)
{
   ...
   ft::AttentionType attention_type = ft::getAttentionType<T>(size_per_head_,
                                                              ...
                                                              true);  // causal_mask
   auto              gpt            = std::make_unique<ft::LlamaV2<T>>(
       ft::LlamaV2<T>(head_num_,
                    kv_head_num_,
                    size_per_head_,
                    inter_size_,
                    num_layer_,
...
void LlamaTritonModel<T>::createSharedWeights(int device_id, int rank)
{
   ...
   shared_weights_[device_id]   = std::make_shared<ft::LlamaV2Weight<T>>(head_num_,
                                                                     kv_head_num_,
                                                                     size_per_head_,
                                                                     inter_size_,
                                                                     vocab_size_,
