                                                   ...
                                                   const float rope_scaling)
{
    ...
    if (!neox_rotary_style) {
        mmha::apply_rotary_embedding(q, k, tidx, rotary_embedding_dim, dst_kv_seq_idx, rope_theta, rope_scaling);
    }
    else {
        ...
        if (do_rotary) {
            ...
            mmha::apply_rotary_embedding(q, k, transpose_idx / tidx_factor, rotary_embedding_dim, dst_kv_seq_idx, rope_theta, rope_scaling);
...
template<typename T, bool PREFIX_PROMPT>
__global__ void add_GQA_fusedQKV_bias_transpose_kernel(T*                               q_buf,
                                                       T*                               k_buf,
                                                       T*                               v_buf,
                                                       PrefixPromptBatchWeightsParam<T> param,
                                                       T*                               QKV,
...
//    const int n                    = head_num * size_per_head;
    ...
    if (!neox_rotary_style) {
        if (head_idx < kv_head_num) {
            ...
            mmha::apply_rotary_embedding(q, tidx, rotary_embedding_dim, dst_kv_seq_idx, rope_theta, rope_scaling);
        }
    }
    else {
        const bool do_rotary = !is_masked && vec_size * tidx < rotary_embedding_dim;
        ...
        if (do_rotary) {
            if (head_idx < head_num) {
                ...
                mmha::vec_from_smem_transpose(q, q_smem, transpose_idx, smem_pitch);
                mmha::apply_rotary_embedding(q, transpose_idx / tidx_factor, rotary_embedding_dim, dst_kv_seq_idx, rope_theta, rope_scaling);
                mmha::write_smem_transpose(q, q_smem, transpose_idx, smem_pitch);
...
#define GQA_FUSED_QKV_BIAS_TRANSPOSE_LAUNCH(T, PREFIX_PROMPT)                                                              \
    add_GQA_fusedQKV_bias_transpose_kernel<T, PREFIX_PROMPT><<<grid, block, smem_size, stream>>>(q_buf,                    \
                                                                                                 k_buf,                    \
                                                                                                 v_buf,                    \
                                                                                                 param,                    \
                                                                                                 QKV,                      \
...
template<typename T>
void invokeGqaAddFusedQKVBiasTranspose(T*                               q_buf,
                                       T*                               k_buf,
                                       T*                               v_buf,
                                       PrefixPromptBatchWeightsParam<T> param,
                                       T*                               QKV,
                                       ...
                                       cudaStream_t                     stream)
{
    ...
    if (rotary_embedding_dim == 0 && param.max_prefix_prompt_length == 0) {
        ...
        //      rotary_embedding_dim);
        if (param.max_prefix_prompt_length == 0) {
            GQA_FUSED_QKV_BIAS_TRANSPOSE_LAUNCH(T, false);
        }
        else {
            GQA_FUSED_QKV_BIAS_TRANSPOSE_LAUNCH(T, true);
        }
    }
}

#define GQAINSTANTIATEADDFUSEDQKVBIASTRANSPOSE(T)                                                                         \
    template void invokeGqaAddFusedQKVBiasTranspose(T*                               q_buf,                               \
                                                    T*                               k_buf,                               \
                                                    T*                               v_buf,                               \
                                                    PrefixPromptBatchWeightsParam<T> param,                               \
                                                    T*                               QKV,                                 \
                                                    ...
                                                    const int                        int8_mode,                           \
                                                    cudaStream_t                     stream)
GQAINSTANTIATEADDFUSEDQKVBIASTRANSPOSE(float);
GQAINSTANTIATEADDFUSEDQKVBIASTRANSPOSE(half);
#ifdef ENABLE_BF16
GQAINSTANTIATEADDFUSEDQKVBIASTRANSPOSE(__nv_bfloat16);
#endif
#undef GAQINSTANTIATEADDFUSEDQKVBIASTRANSPOSE
